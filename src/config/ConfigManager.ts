...
export class ConfigManager {
   ...
   */
  public async loadConfig(configPath: string): Promise<AppConfig> {
    try {
      ...
      const configError = new WebToAPKError(
        'CONFIG',
   ...
   */
  public createDefaultConfig(outputPath: string): AppConfig {
    try {
      ...
      const configError = new WebToAPKError(
        'CONFIG',
        `Failed to create default configuration: ${error instanceof Error ? error.message : String(error)}`,
   ...
   */
  public generateCapacitorConfig(config: AppConfig): CapacitorConfig {
    try {
      ...
      const configError = new WebToAPKError(
        'CONFIG',
        `Failed to generate Capacitor configuration: ${error instanceof Error ? error.message : String(error)}`,
   ...
   */
  public async saveCapacitorConfig(capacitorConfig: CapacitorConfig, outputPath: string): Promise<void> {
    try {
...
`;
      ...
      const configError = new WebToAPKError(
        'CONFIG',
        `Failed to save Capacitor configuration: ${error instanceof Error ? error.message : String(error)}`,
        { path: outputPath, originalError: error }
      );
   ...
   */
  public mergeConfigs(baseConfig: AppConfig, overrideConfig: Partial<AppConfig>): AppConfig {
    const merged: AppConfig = { ...baseConfig };

    // Merge simple properties
    Object.keys(overrideConfig).forEach(key => {
      const value = overrideConfig[key as keyof AppConfig];
      if (value !== undefined) {
   ...
   */
  public getErrorSolution(error: WebToAPKError): string {
   ...
   */
  private handleValidationResult(validationResult: ValidationResult, configPath: string): void {
    // Log warnings
   ...
   */
  public resolveConfigPaths(config: AppConfig, configPath: string): AppConfig {
