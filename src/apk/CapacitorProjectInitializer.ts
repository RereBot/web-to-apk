...
import { AppConfig, CapacitorConfig, WebToAPKError } from '../types/index.js';
 ...
 */
export class CapacitorProjectInitializerImpl implements CapacitorProjectInitializer {
  private resourceProcessor: ResourceProcessorImpl;
   ...
   */
  async createProject(config: AppConfig, projectPath: string): Promise<void> {
    try {
      ...
      throw new WebToAPKError(
        'BUILD',
        `Failed to create Capacitor project: ${error instanceof Error ? error.message : String(error)}`,
        { projectPath, config }
      );
    }
  }
   ...
   */
  async generateCapacitorConfig(config: AppConfig, projectPath: string): Promise<void> {
...
`;
   ...
   */
  async addAndroidPlatform(projectPath: string): Promise<void> {
    try {
      ...
      throw new WebToAPKError(
        'BUILD',
        `Failed to add Android platform: ${errorMessage}`,
        {
          projectPath,
   ...
   */
  private async verifyAndroidEnvironment(): Promise<void> {
    const errors: string[] = [];
    ...
    if (errors.length > 0) {
      throw new WebToAPKError(
        'BUILD',
        'Android development environment is not properly configured',
        { errors }
      );
    }
  }
   ...
   */
  async installDependencies(projectPath: string, plugins: string[] = []): Promise<void> {
    try {
      const originalCwd = process.cwd();
      process.chdir(projectPath);
   ...
   */
  private async generatePackageJson(config: AppConfig, projectPath: string): Promise<void> {
    const packageJson = {
   ...
   */
  private async copyWebAssets(sourceDir: string, targetDir: string): Promise<void> {
    try {
   ...
   */
  private async processResources(config: AppConfig, projectPath: string): Promise<void> {
    try {
      ...
      if (config.icon) {
        ...
        try {
          ...
          console.warn(
            `Warning: Failed to process icon: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      }
      ...
      if (config.splashScreen) {
        ...
        try {
          await this.resourceProcessor.generateSplashScreens(
            config.splashScreen,
            androidResourcesPath
          );
        } catch (error) {
          console.warn(
            `Warning: Failed to process splash screen: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      }
    } catch (error) {
      ...
      console.warn(
        `Warning: Resource processing failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
