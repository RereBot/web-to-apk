import { APKSigner, DebugKeystoreConfig } from '../interfaces/APKSigner.js';
import { KeystoreConfig, WebToAPKError } from '../types/index.js';
 ...
 */
export class APKSignerImpl implements APKSigner {
  private static readonly DEBUG_KEYSTORE_CONFIG: DebugKeystoreConfig = {
   ...
   */
  async signAPK(apkPath: string, keystoreConfig: KeystoreConfig): Promise<string> {
    ...
    try {
      ...
      throw new WebToAPKError(
        'SIGNING',
        `Failed to sign APK: ${error instanceof Error ? error.message : String(error)}`,
        { apkPath, keystoreConfig: { ...keystoreConfig, password: '***', aliasPassword: '***' } }
      );
    }
  }
   ...
   */
  async signAPKWithDebugKeystore(apkPath: string): Promise<string> {
    try {
      ...
      const debugKeystoreConfig: KeystoreConfig = {
      ...
      throw new WebToAPKError(
        'SIGNING',
        `Failed to sign APK with debug keystore: ${error instanceof Error ? error.message : String(error)}`,
        { apkPath }
      );
    }
  }
   ...
   */
  async verifyAPKSignature(apkPath: string): Promise<boolean> {
    try {
      await this.validateAPKPath(apkPath);
      ...
      return new Promise((resolve, reject) => {
        ...
        verifyProcess.on('error', error => {
          reject(
            new WebToAPKError(
              'SIGNING',
              `Failed to start APK signature verification: ${error.message}`,
              { apkPath, error: error.message }
            )
          );
        });
      });
      ...
      throw new WebToAPKError(
        'SIGNING',
        `APK signature verification error: ${error instanceof Error ? error.message : String(error)}`,
        { apkPath }
      );
    }
  }
   ...
   */
  async generateDebugKeystore(keystorePath: string): Promise<string> {
    try {
      ...
      throw new WebToAPKError(
        'SIGNING',
        `Failed to generate debug keystore: ${error instanceof Error ? error.message : String(error)}`,
        { keystorePath }
      );
    }
  }
   ...
   */
  async validateKeystoreConfig(keystoreConfig: KeystoreConfig): Promise<boolean> {
    try {
      // Check required fields
      const requiredFields: (keyof KeystoreConfig)[] = [
        'path',
        'password',
        'alias',
        'aliasPassword'
      ...
      if (missingFields.length > 0) {
        throw new WebToAPKError(
          'SIGNING',
          `Missing required keystore configuration fields: ${missingFields.join(', ')}`,
          {
            missingFields,
            keystoreConfig: { ...keystoreConfig, password: '***', aliasPassword: '***' }
          }
      ...
      throw new WebToAPKError(
        'SIGNING',
        `Keystore validation failed: ${error instanceof Error ? error.message : String(error)}`,
        { keystoreConfig: { ...keystoreConfig, password: '***', aliasPassword: '***' } }
      );
    }
  }
   ...
   */
  private async validateAPKPath(apkPath: string): Promise<void> {
    try {
      ...
      throw new WebToAPKError(
        'SIGNING',
        `APK file validation failed: ${error instanceof Error ? error.message : String(error)}`,
        { apkPath }
      );
    }
  }
   ...
   */
  private generateSignedAPKPath(originalApkPath: string): string {
   ...
   */
  private async executeAPKSigner(
    inputApkPath: string,
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(
        `Signing APK: ${apksignerPath} sign --ks [keystore] --ks-key-alias [alias] --out ${outputApkPath} ${inputApkPath}`
      );
      ...
      signingProcess.stdout?.on('data', (data: Buffer) => {
        stdout += data.toString();
      });

      signingProcess.stderr?.on('data', (data: Buffer) => {
        stderr += data.toString();
      });
      ...
      signingProcess.on('close', code => {
        if (code === 0) {
          ...
          reject(
            new WebToAPKError(
              'SIGNING',
              `APK signing failed with exit code ${code}: ${errorMessage}`,
              {
                exitCode: code,
      ...
      signingProcess.on('error', error => {
        reject(
          new WebToAPKError('SIGNING', `Failed to start APK signing process: ${error.message}`, {
            error: error.message,
            apksignerPath
          })
        );
   ...
   */
  private async executeKeytool(keystorePath: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      console.log(
        `Generating debug keystore: ${keytoolPath} -genkeypair -keystore ${keystorePath}`
      );
      ...
      keytoolProcess.on('close', code => {
        if (code === 0) {
          ...
          reject(
            new WebToAPKError(
              'SIGNING',
              `Keystore generation failed with exit code ${code}: ${stderr || stdout}`,
              {
                exitCode: code,
                stdout: stdout.slice(-500),
                stderr: stderr.slice(-500),
      ...
      keytoolProcess.on('error', error => {
        reject(
          new WebToAPKError('SIGNING', `Failed to start keytool process: ${error.message}`, {
            error: error.message,
            keytoolPath
          })
        );
   ...
   */
  private getAPKSignerPath(): string {
    const androidHome = process.env.ANDROID_HOME || process.env.ANDROID_SDK_ROOT;
    if (!androidHome) {
      throw new WebToAPKError(
        'SIGNING',
        'ANDROID_HOME or ANDROID_SDK_ROOT environment variable not set',
        { availableEnvVars: Object.keys(process.env).filter(key => key.includes('ANDROID')) }
      );
    }
    ...
    throw new WebToAPKError(
      'SIGNING',
      'apksigner tool not found in Android SDK. Please ensure Android SDK build-tools are installed.',
      { androidHome, searchedPaths: possiblePaths }
    );
  }
   ...
   */
  private async ensureDebugKeystore(): Promise<string> {
   ...
   */
  private parseSigningError(stderr: string, stdout: string): string {
    ...
    const errorLines = combinedOutput
      .split('\n')
      .filter(
        line => line.includes('ERROR:') || line.includes('Exception:') || line.includes('Error:')
      );
