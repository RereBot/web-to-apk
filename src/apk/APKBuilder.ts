import { AppConfig, BuildOptions, WebToAPKError, KeystoreConfig } from '../types/index.js';
 ...
 */
export class APKBuilderImpl implements APKBuilder {
  private projectInitializer: CapacitorProjectInitializerImpl;
  private apkSigner: APKSignerImpl;
   ...
   */
  async initializeProject(config: AppConfig, projectPath: string): Promise<void> {
    try {
   ...
   */
  async buildAPK(projectPath: string, options: BuildOptions): Promise<string> {
    try {
      ...
      throw new WebToAPKError(
        'BUILD',
        `Failed to build APK: ${error instanceof Error ? error.message : String(error)}`,
        { projectPath, options }
      );
    }
  }
  ...
  private validateConfig(config: AppConfig): void {
    ...
    if (!/^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)+$/.test(config.packageName)) {
      throw new WebToAPKError(
        'CONFIG',
        'Invalid package name format. Must be in format com.example.app',
    ...
    try {
      ...
      for (const file of requiredFiles) {
        ...
        try {
          ...
          throw new WebToAPKError(
            'BUILD',
            `Required file not found: ${file}. Please ensure the project is properly initialized.`,
            { projectPath, missingFile: file }
          );
        }
      }
      ...
      try {
        ...
        throw new WebToAPKError(
          'BUILD',
          'Android platform not found. Please run project initialization first.',
          { projectPath }
        );
      }
    } catch (error) {
   ...
   */
  private async executeGradleBuild(
    projectPath: string,
    gradleTask: string,
    ...
    return new Promise((resolve, reject) => {
      ...
      gradleProcess.on('close', code => {
        if (code === 0) {
          ...
          reject(
            new WebToAPKError(
              'BUILD',
              `Gradle build failed with exit code ${code}: ${errorMessage}`,
              {
                exitCode: code,
      ...
      gradleProcess.on('error', error => {
        reject(
          new WebToAPKError('BUILD', `Failed to start Gradle process: ${error.message}`, {
            error: error.message,
            gradleCommand,
            androidPath
          })
      ...
      const timeout = setTimeout(
        () => {
          ...
          reject(
            new WebToAPKError('BUILD', 'Gradle build timed out after 10 minutes', {
              gradleTask,
              projectPath
            })
          );
   ...
   */
  private parseGradleError(stderr: string, stdout: string): string {
    ...
    const errorPatterns = [
      ...
      },
      {
        ...
        message:
          'Android SDK not found. Please install Android SDK and set ANDROID_HOME environment variable.'
      },
      {
    ...
    const errorLines = combinedOutput
      .split('\n')
      .filter(
        line => line.includes('ERROR:') || line.includes('FAILURE:') || line.includes('Exception:')
      );
   ...
   */
  private async findBuiltAPK(projectPath: string, buildType: string): Promise<string> {
    ...
    try {
      ...
      throw new WebToAPKError(
        'BUILD',
        `Failed to find built APK: ${error instanceof Error ? error.message : String(error)}`,
        { apkDir, buildType }
      );
    }
  }
  ...
  private async copyAPKToOutput(
    ...
    try {
      ...
      throw new WebToAPKError(
        'BUILD',
        `Failed to copy APK to output directory: ${error instanceof Error ? error.message : String(error)}`,
        { apkPath, outputDir, buildType }
      );
    }
  }
}
