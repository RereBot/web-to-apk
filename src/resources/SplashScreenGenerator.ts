 ...
 */
export class SplashScreenGenerator implements ISplashScreenGenerator {
  private readonly ANDROID_SPLASH_SIZES: SplashScreenSize[] = [
   ...
   */
  async processSplashScreen(options: SplashScreenOptions): Promise<ProcessedSplashScreen[]> {
    const {
      inputPath,
      outputDir,
    ...
    const results: ProcessedSplashScreen[] = [];
    ...
    for (const size of sizesToGenerate) {
      try {
        ...
        const fileName =
          size.orientation === 'portrait' ? 'splash_portrait.png' : 'splash_landscape.png';
        const outputPath = join(outputFolder, fileName);

        await this.generateSplashScreen(
          inputPath,
          outputPath,
          size.width,
          size.height,
   ...
   */
  async validateSplashScreen(splashPath: string): Promise<boolean> {
    try {
      ...
      if (metadata.width < 480 || metadata.height < 800) {
        console.warn(
          `Warning: Splash screen ${splashPath} is smaller than recommended size (480x800). Current size: ${metadata.width}x${metadata.height}`
        );
      }
  ...
  getRequiredSizes(): SplashScreenSize[] {
    ...
    return this.ANDROID_SPLASH_SIZES.filter(size =>
  ...
  async generateSplashScreen(
    ...
    try {
      ...
      const sharpFitMode =
        scaleMode === 'fill' ? 'fill' : scaleMode === 'cover' ? 'cover' : 'contain';
      ...
      throw new WebToAPKError(
        'RESOURCE',
        `Failed to generate splash screen: ${error instanceof Error ? error.message : 'Unknown error'}`,
        {
          inputPath,
          outputPath,
          width,
  ...
  async getSplashScreenMetadata(
    ...
    try {
      ...
      throw new WebToAPKError(
        'RESOURCE',
        `Failed to read splash screen metadata: ${error instanceof Error ? error.message : 'Unknown error'}`,
        { splashPath }
      );
    }
  }
