...
export class CLIImpl implements CLI {
  ...
  async build(configPath: string, options: BuildOptions): Promise<void> {
    ...
    try {
      ...
      if (reporter!) {
        reporter.completeBuild(false, undefined, undefined, error as Error);
        if (options.verbose) {
          reporter.displayDetailedReport();
        } else {
          reporter.displaySummary();
   ...
   * 交互式配置提示
   */
  private async promptForConfig(options: InitOptions): Promise<Partial<AppConfig>> {
    ...
    const basicAnswers = await inquirer.prompt([
      {
        ...
        validate: (input: string) => input.trim().length > 0 || '应用名称不能为空'
      },
      {
        type: 'input',
        name: 'packageName',
    ...
    const webAnswers = await inquirer.prompt([
      {
        ...
        message: 'Web文件目录:',
        default: './dist',
        validate: (input: string) => input.trim().length > 0 || 'Web目录不能为空'
      },
      {
        type: 'input',
        name: 'startUrl',
  ...
  private createDefaultConfig(options: InitOptions): Partial<AppConfig> {
    return {
   ...
   * 检查项目目录
   */
  private async checkProjectDirectory(projectPath: string, force?: boolean): Promise<void> {
    try {
   ...
   * 创建资源目录
   */
  private async createResourceDirectories(projectPath: string): Promise<void> {
  ...
  private displayBuildInfo(config: Partial<AppConfig>, options: BuildOptions): void {
    console.log(chalk.blue('\nBuild information:'));
  ...
  private async cleanOutputDirectory(outputDir: string): Promise<void> {
    try {
      ...
      console.warn(
        chalk.yellow(`⚠️  清理输出目录失败: ${error instanceof Error ? error.message : error}`)
      );
    }
  }

  private async processResources(config: Partial<AppConfig>, _projectPath: string): Promise<void> {
    // 这里可以集成图标处理器和启动画面生成器
    // 暂时只是一个占位符实现
    if (config.icon) {
  ...
  private handleBuildError(error: unknown): void {
    if (error instanceof Error) {
      ...
      // 提供解决建议
      const solution = this.errorHandler.provideSolution(error as any);
      if (solution) {
  ...
  private generateDevelopmentGuide(_config: Partial<AppConfig>, template: any): string {
    return `# 开发指南
...
`;
  ...
  private generateDeploymentGuide(_config: Partial<AppConfig>): string {
    return `# 部署指南
