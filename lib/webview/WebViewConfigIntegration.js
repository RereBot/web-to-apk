...
export class WebViewConfigIntegration {
     ...
     * 从应用配置创建WebView配置
     */
    createWebViewConfigFromAppConfig(appConfig) {
     ...
     * 将WebView权限合并到应用配置中
     */
    async mergeWebViewPermissionsToAppConfig(appConfig, webViewConfig) {
        const webViewPermissions = await this.webViewConfigManager.generatePermissionsManifest(webViewConfig.permissions);
        // 合并权限，避免重复
     ...
     * 生成Capacitor配置文件内容
     */
    async generateCapacitorConfigFile(appConfig, webViewConfig) {
        const baseCapacitorConfig = await this.webViewConfigManager.generateCapacitorConfig(webViewConfig);
        // 添加应用特定的配置
     ...
     */
    async validateCompatibility(appConfig, webViewConfig) {
        const errors = [];
        // 验证WebView配置本身
        ...
        // 验证权限兼容性
        if (webViewConfig.permissions.internet &&
        ...
        // 验证导航规则与应用URL的兼容性
        if (appConfig.startUrl) {
            const isStartUrlAllowed = this.isUrlAllowedByNavigationRules(appConfig.startUrl, webViewConfig.navigationRules);
     ...
     * 将应用权限映射到WebView权限
     */
    mapAppPermissionsToWebViewPermissions(appPermissions) {
        const permissions = {
            internet: false,
            networkState: false,
     ...
     * 根据应用配置生成导航规则
     */
    generateNavigationRules(appConfig) {
        const rules = [
            // 默认规则
            {
                pattern: 'file://*',
     ...
     * 检查URL是否被导航规则允许
     */
    isUrlAllowedByNavigationRules(url, rules) {
...
//# sourceMappingURL=WebViewConfigIntegration.js.map