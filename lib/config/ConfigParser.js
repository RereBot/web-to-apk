...
export class ConfigParser {
    ...
    loadConfig(configPath) {
        try {
            ...
            throw new WebToAPKError('CONFIG', `Failed to load configuration: ${error instanceof Error ? error.message : String(error)}`, { path: configPath, originalError: error });
        }
    }
    /**
    ...
    saveConfig(config, outputPath) {
        try {
            ...
            throw new WebToAPKError('CONFIG', `Failed to save configuration: ${error instanceof Error ? error.message : String(error)}`, { path: outputPath, originalError: error });
        }
    }
}
