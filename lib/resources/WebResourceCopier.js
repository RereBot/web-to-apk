 ...
 */
export class WebResourceCopier {
    constructor() {
        ...
        this.TRANSFORMABLE_EXTENSIONS = ['.html', '.htm', '.css', '.js'];
    }
    /**
     * Copy web resources from source directory to target directory
     */
    async copyWebResources(sourceDir, targetDir) {
        const result = {
            success: true,
            copiedFiles: [],
            errors: [],
     ...
     */
    async validateWebResources(webDir) {
        const result = {
            isValid: true,
            errors: [],
            warnings: []
     ...
     */
    async transformResourcePaths(filePath, transformOptions) {
        const options = {
            baseUrl: '',
            assetPrefix: '',
        ...
        try {
            ...
            throw new WebToAPKError('RESOURCE', `Failed to transform paths in ${filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`, { filePath, transformOptions });
        }
    }
    /**
     * Get list of all web resource files in directory
     */
    async getWebResourceFiles(webDir) {
        const files = [];
        try {
            await this.scanDirectory(webDir, files);
        }
        catch (error) {
            throw new WebToAPKError('RESOURCE', `Failed to scan web directory: ${error instanceof Error ? error.message : 'Unknown error'}`, { webDir });
        }
        return files;
    }
    /**
     ...
     */
    transformHtmlPaths(content, options) {
        // Transform src attributes
        content = content.replace(/\s(src|href)=["']([^"']+)["']/gi, (match, attr, path) => this.transformPath(match, attr, path, options));
        // Transform CSS url() references in style attributes
        content = content.replace(/style=["'][^"']*url\(([^)]+)\)[^"']*/gi, match => this.transformCssUrlsInMatch(match, options));
        return content;
    }
    /**
     * Transform paths in CSS content
     */
    transformCssPaths(content, options) {
     ...
     */
    transformJsPaths(content, options) {
        ...
        content = content.replace(/(require\(["']|import\s+.*?from\s+["']|import\s*\(["'])([^"']+)(["'])/gi, (match, prefix, path, suffix) => {
     ...
     */
    transformPath(_match, attr, path, options) {
        const transformedPath = this.applyPathTransform(path, options);
     ...
     */
    transformCssUrlsInMatch(match, options) {
     ...
     */
    applyPathTransform(path, options) {
...
//# sourceMappingURL=WebResourceCopier.js.map