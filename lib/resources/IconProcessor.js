...
export class IconProcessor {
    constructor() {
        ...
        this.SUPPORTED_FORMATS = ['png', 'jpg', 'jpeg', 'webp', 'gif', 'svg'];
    }
    /**
    ...
    async validateIcon(iconPath) {
        try {
            ...
            if (metadata.width < 192 || metadata.height < 192) {
                console.warn(`Warning: Icon ${iconPath} is smaller than recommended size (192x192). Current size: ${metadata.width}x${metadata.height}`);
            }
    ...
    getRequiredSizes() {
        ...
        return this.ANDROID_ICON_SIZES.filter(size => ['mdpi', 'hdpi', 'xhdpi', 'xxhdpi', 'xxxhdpi'].includes(size.density) &&
            size.folder.startsWith('mipmap'));
    }
    /**
     * Resize an icon to specific dimensions
     */
    async resizeIcon(inputPath, outputPath, width, height) {
        try {
            ...
            throw new WebToAPKError('RESOURCE', `Failed to resize icon: ${error instanceof Error ? error.message : 'Unknown error'}`, { inputPath, outputPath, width, height });
        }
    }
    /**
     * Get icon metadata information
     */
    async getIconMetadata(iconPath) {
        try {
            const metadata = await sharp(iconPath).metadata();
            return {
            ...
            throw new WebToAPKError('RESOURCE', `Failed to read icon metadata: ${error instanceof Error ? error.message : 'Unknown error'}`, { iconPath });
        }
    }
}
//# sourceMappingURL=IconProcessor.js.map