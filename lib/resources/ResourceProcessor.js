 ...
 */
export class ResourceProcessorImpl {
    constructor() {
    ...
    async processIcon(iconPath, outputDir) {
        try {
            ...
            this.logger.info(`Successfully processed icon: ${results.length} sizes generated`, 'RESOURCE', {
                iconPath,
                outputDir,
            ...
            throw new WebToAPKError('RESOURCE', `Failed to process icon: ${error instanceof Error ? error.message : String(error)}`, { iconPath, outputDir });
        }
    }
    /**
     * Generate splash screen resources for different screen densities
     */
    async generateSplashScreens(splashPath, outputDir) {
        try {
            ...
            if (failedResults.length > 0) {
                const errors = failedResults.map(result => result.error).join(', ');
                throw new WebToAPKError('RESOURCE', `Failed to generate some splash screen sizes: ${errors}`, { splashPath, outputDir, failedResults });
            }
            this.logger.info(`Successfully generated splash screens: ${results.length} sizes created`, 'RESOURCE', {
                splashPath,
                outputDir,
            ...
            throw new WebToAPKError('RESOURCE', `Failed to generate splash screens: ${error instanceof Error ? error.message : String(error)}`, { splashPath, outputDir });
        }
    }
    /**
     * Copy web assets to the output directory
     */
    async copyWebAssets(webDir, outputDir) {
        try {
            ...
            this.logger.info(`Successfully copied web assets: ${result.copiedFiles.length} files (${result.totalSize} bytes)`, 'RESOURCE', {
                webDir,
                outputDir,
            ...
            throw new WebToAPKError('RESOURCE', `Failed to copy web assets: ${error instanceof Error ? error.message : String(error)}`, { webDir, outputDir });
        }
    }
}
