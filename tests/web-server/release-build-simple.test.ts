/**
 * 发布版签名功能简单测试
 * 直接测试server.js中的buildAPK函数逻辑
 */

import fs from 'fs/promises';
import path from 'path';

describe('发布版签名功能简单测试', () => {
  ...
  describe('核心功能：构建命令参数验证', () => {
    ...
    test('当keyPassword为空时，应该使用keystorePassword作为key-password', async () => {
      delete require.cache[require.resolve('../../web-server/server.js')];
      const serverModule = require('../../web-server/server.js');
      const buildAPK = serverModule.buildAPK;
